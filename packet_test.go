...
func TestMultipleUnmarshals(t *testing.T) {
	cases := map[string]struct {
		rawPkt         []byte
		expectedPacket *Packet
	}{
		"PacketOneByteExtension": {
			rawPkt: []byte{
				0x90, 0xe0, 0x69, 0x8f, 0xd9, 0xc2, 0x93, 0xda, 0x1c, 0x64,
				0x27, 0x82, 0xBE, 0xDE, 0x00, 0x01, 0x50, 0xAA, 0x00, 0x00,
				0x98, 0x36, 0xbe, 0x88, 0x9e,
		...
		"PacketMultipleOneByteExtensions": {
			rawPkt: []byte{
				0x90, 0xe0, 0x69, 0x8f, 0xd9, 0xc2, 0x93, 0xda, 0x1c, 0x64,
				0x27, 0x82, 0xBE, 0xDE, 0x00, 0x03, 0x10, 0xAA, 0x21, 0xBB,
				0xBB, 0x33, 0xCC, 0xCC, 0xCC, 0xCC, 0x00, 0x00, 0x98, 0x36,
				0xbe, 0x88, 0x9e,
			},
			expectedPacket: &Packet{Header: Header{
				Marker:           true,
		...
		"PacketMultipleTwoByteExtensions": {
			rawPkt: []byte{
				0x90, 0xe0, 0x69, 0x8f, 0xd9, 0xc2, 0x93, 0xda, 0x1c, 0x64,
				0x27, 0x82, 0x10, 0x00, 0x00, 0x03, 0x01, 0x00, 0x02, 0x01,
				0xBB, 0x00, 0x03, 0x04, 0xCC, 0xCC, 0xCC, 0xCC, 0x98, 0x36,
	...
	for name, testCase := range cases {
		t.Run(name, func(t *testing.T) {
			p := &Packet{}
			// First Unmarshal
			if err := p.Unmarshal(testCase.rawPkt); err != nil {
				t.Error("Test multiple unmarshal: first: %w", err)
			} else if len(p.Extensions) != len(testCase.expectedPacket.Extensions) {
				t.Errorf("Test multiple unmarshal: first: got %d extensions, want %d", len(p.Extensions), len(testCase.expectedPacket.Extensions))
			} else if !reflect.DeepEqual(p, testCase.expectedPacket) {
				t.Errorf("TestMultiple unmarshal: first: got %#v, want %#v", p, testCase.expectedPacket)
			}

			// Second Unmarshal
			if err := p.Unmarshal(testCase.rawPkt); err != nil {
				t.Error("Test multiple unmarshal: second: %w", err)
			} else if len(p.Extensions) != len(testCase.expectedPacket.Extensions) {
				t.Errorf("Test multiple unmarshal: second: got %d extensions, want %d", len(p.Extensions), len(testCase.expectedPacket.Extensions))
			} else if !reflect.DeepEqual(p, testCase.expectedPacket) {
				t.Errorf("TestMultiple unmarshal: second: got %#v, want %#v", p, testCase.expectedPacket)
			}
		})
...
func TestUnmarshalExtensionThenPacketWithoutExtension(t *testing.T) {
	pktWithExtensions := []byte{
		0x90, 0xe0, 0x69, 0x8f, 0xd9, 0xc2, 0x93, 0xda, 0x1c, 0x64,
		0x27, 0x82, 0xBE, 0xDE, 0x00, 0x01, 0x50, 0xAA, 0x00, 0x00,
		0x98, 0x36, 0xbe, 0x88, 0x9e,
	}

	pktWithoutExtensions := []byte{
		0x80, 0xe0, 0x69, 0x8f, 0xd9, 0xc2, 0x93, 0xda, 0x1c, 0x64,
		0x27, 0x82, 0x00, 0x01, 0x98, 0x36, 0xbe, 0x88, 0x9e,
	}
	...
	parsedPacket := &Packet{Header: Header{
		...
		Payload: pktWithoutExtensions[12:],
		Raw:     pktWithoutExtensions,
	}

	p := &Packet{}

	if err := p.Unmarshal(pktWithExtensions); err != nil {
		t.Error(err)
	}
